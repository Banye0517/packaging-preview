...
describe('ExportFrameOverlay', () => {
  it('shows the fixed frame size and switches aspect ratios', async () => {
    ...
    render(<ExportFrameOverlay presetId="square-standard" onChange={onChange} pedestal={{ preset: 'none', color: 'warm-white', cornerRadiusMm: 8 }} onPedestalPresetChange={vi.fn()} onPedestalColorChange={vi.fn()} />)
  ...
  it('switches pedestal preset and color without adding a settings area', async () => {
    ...
    const onColor = vi.fn()
    render(<ExportFrameOverlay presetId="square-standard" onChange={vi.fn()} pedestal={{ preset: 'steps', color: 'warm-white', cornerRadiusMm: 8 }} onPedestalPresetChange={onPreset} onPedestalColorChange={onColor} />)
