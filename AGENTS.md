...
- Hanging-tissue body radius defaults to `0 mm` and is capped at half the smaller current width/depth. It rounds only the middle-body cross-section and fades out through the existing top connector range; never deform the handle or independent pulled sheet, and never alter artwork transforms or artwork-size references.
